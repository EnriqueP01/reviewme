<nav x-data="{ open: false }" class="bg-surface/40 backdrop-blur-3xl sticky top-0 z-50 border-b border-subtle h-24 flex items-center transition-all duration-500">
    <div class="max-w-7xl mx-auto px-12 w-full">
        <div class="flex justify-between items-center">
            ...
            <div class="hidden sm:flex items-center gap-10">
                ...
                @else
                    <div class="flex items-center gap-6">
                        <a href="{{ route('login', ['mode' => 'login']) }}" wire:navigate class="text-[10px] font-black uppercase tracking-widest text-on-surface-variant hover:text-white transition-colors">{{ __('Sign In') }}</a>
                        <x-ui.button variant="primary" size="sm" href="{{ route('register', ['mode' => 'register']) }}" wire:navigate>
                            {{ __('Get Started') }}
                        </x-ui.button>
                    </div>
                @endauth
    ...
    <div :class="{'translate-x-0': open, 'translate-x-full': !open}" class="fixed inset-y-0 right-0 w-80 bg-surface-container-highest/95 backdrop-blur-2xl z-[60] border-l border-white/5 transition-transform duration-500 transform ease-in-out sm:hidden">
        <div class="p-10 space-y-10">
            ...
            <div class="space-y-6">
                 <a href="{{ route('dashboard') }}" wire:navigate class="block text-3xl font-display font-black text-on-surface hover:text-primary transition-colors">{{ __('Feed') }}</a>
            </div>
            <div class="pt-10 border-t border-white/5">
                <a href="{{ route('publish') }}" wire:navigate class="block">
