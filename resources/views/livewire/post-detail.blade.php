                                ...
                                <div class="bg-[#1e1f2b] rounded-3xl p-6 border border-white/5 shadow-xl transition-all hover:border-primary/20" x-data="{ expanded: false }">
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center gap-4">
                                            ...
                                            @if($reviewFilesData[$snippet->id]['modified'])
            ...
            <div class="bg-[#1a1b26] rounded-[3rem] overflow-hidden border border-white/5 shadow-[0_40px_120px_rgba(0,0,0,0.7)] relative group/viewer">
                ...
                <div class="flex items-center justify-between px-10 py-6 bg-[#1e1f2b] border-b border-white/5">
                    <div class="flex items-center gap-8">
                        ...
                        <div class="flex flex-col">
                            <span class="text-xs text-on-surface font-mono font-bold tracking-tight">{{ $currentSnippet->filename ?: $currentSnippet->name }}</span>
                            <span class="text-[8px] font-black uppercase tracking-[0.3em] text-on-surface-variant/40 line-clamp-1">{{ $currentSnippet->description ?: 'Source implementation' }}</span>
                        </div>
                    </div>
                    <div class="flex items-center gap-4">
                        <span class="text-[10px] uppercase font-black tracking-[0.3em] text-primary/70 px-5 py-2 bg-primary/5 rounded-2xl border border-primary/20 shadow-sm backdrop-blur-sm">{{ $currentSnippet->language }}</span>
                    </div>
                </div>

                <div class="p-0 font-mono text-[14px] leading-relaxed overflow-x-auto bg-[#0d0e12] py-8 custom-scrollbar">
                    <table class="w-full border-separate border-spacing-0">
                        @foreach(explode("\n", $currentSnippet->code_content) as $index => $line)
                            @php 
                                $num = $index + 1; 
                                $suggestion = $currentSnippet->inlineSuggestions->where('line_number', $num)->first();
                            ...
                            <tr class="group hover:bg-white/[0.03] transition-colors relative" 
                                ...
                                :class="{ 'bg-primary/5': activeLine == {{ $num }} }">
                                ...
                                <td class="whitespace-pre px-8 group/line relative align-top py-1" @mouseup="handleMouseUp($event, {{ $num }}, @js($line))">
                                    ...
                                    <button wire:click="selectLine({{ $num }})" class="absolute right-6 top-1/2 -translate-y-1/2 opacity-0 group-hover/line:opacity-100 p-2 text-primary/40 hover:text-primary transition-all hover:scale-125 focus:outline-none">
                                        ...
                                        <div class="flex items-start justify-between group/review">
                                            <div class="flex gap-8">
                                                <div class="relative flex-shrink-0">
                                                    <img src="{{ $review->user->avatar }}" class="w-12 h-12 rounded-[1.25rem] border-2 border-white/5 shadow-2xl object-cover scale-110 group-hover/review:scale-100 transition-transform duration-500">
            ...
            <div class="space-y-16 pt-32 border-t border-white/5">
                <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
                    ...
                    <div class="flex gap-4">
                         <div class="px-5 py-2 rounded-xl bg-white/5 border border-white/5 backdrop-blur-md">
                             <div class="flex items-center gap-3">
                                 <span class="text-[10px] font-black text-primary">Live</span>
                            ...
                            <div class="relative group/maincmt {{ $comment->is_pinned ? 'bg-primary/[0.04] border border-primary/20 rounded-[2.5rem] p-10 transition-all shadow-[0_20px_50px_rgba(var(--primary-rgb),0.1)]' : '' }}">
                                ...
                                <div class="flex items-start gap-10">
                                    <div class="flex-shrink-0 relative group/avatar">
                                        ...
                                        <div class="absolute -bottom-2 -right-2 w-7 h-7 bg-[#1a1b26] rounded-xl border-2 border-primary/30 flex items-center justify-center text-xs shadow-xl z-20">
                                            🎖️
                                        </div>
                                    </div>
                                    <div class="flex-1 space-y-6">
                                        ...
                                        <!-- Nested Reply Input (YouTube style) -->
                                        <div x-show="replyTo === {{ $comment->id }}" x-transition x-cloak class="pt-8 animate-slide-up relative">
                                            <div class="bg-[#1e1f2b] p-10 rounded-[2.5rem] border border-primary/30 shadow-[0_30px_60px_rgba(0,0,0,0.5)] space-y-6 relative overflow-hidden group/nest">
                                                <div class="absolute top-0 right-0 w-48 h-48 bg-primary/5 blur-[80px] rounded-full pointer-events-none group-hover/nest:scale-150 transition-transform duration-1000"></div>
                                                
                                                <div class="flex items-center gap-4 mb-2 relative">
                                                    <div class="w-2 h-2 rounded-full bg-primary animate-pulse shadow-[0_0_10px_#bec2ff]"></div>
                                                    <span class="text-[10px] font-black uppercase tracking-[0.3em] text-primary">{{ __('Advanced Reply') }} — {{ __('To') }} {{ $comment->user->name }}</span>
                                                </div>
                                ...
                                @if($comment->replies->count() > 0)
                                    <div class="ml-24 mt-12 pl-12 border-l-2 border-white/5 space-y-12 relative">
                                        <div class="absolute left-0 top-0 bottom-0 w-[2px] bg-gradient-to-b from-primary/30 via-primary/5 to-transparent rounded-full opacity-50"></div>
                                        @foreach($comment->replies as $reply)
                                            <div class="flex items-start gap-8 group/reply relative">
                                                ...
                                                <div class="flex-1 space-y-3">
                                                    ...
                                                    <div class="relative pl-6">
                                                        <div class="absolute left-0 top-0 bottom-0 w-1 bg-white/[0.02] rounded-full"></div>
        ...
        <div class="space-y-12 lg:sticky lg:top-16 self-start">
            <div class="bg-[#1a1b26] rounded-[3.5rem] p-12 border border-white/5 shadow-[0_40px_100px_rgba(0,0,0,0.5)] relative overflow-hidden group/sidebar">
                ...
                <div class="grid grid-cols-2 gap-6 relative">
                    <button wire:click="react('clean')" class="group/react flex flex-col items-center p-8 rounded-[2.5rem] border-2 transition-all duration-500 shadow-xl {{ $post->reactions->where('user_id', auth()->id())->where('type', 'clean')->count() ? 'bg-primary/20 border-primary shadow-[0_25px_50px_rgba(var(--primary-rgb),0.3)] scale-105' : 'bg-[#1e1f2b] border-white/5 hover:border-primary/50' }} hover:-translate-y-2 active:scale-95">
                        <span class="text-5xl mb-6 group-hover/react:scale-125 group-hover/react:-rotate-6 transition-transform duration-500 drop-shadow-2xl">✨</span>
                        <span class="text-[10px] font-black uppercase tracking-[0.25em] text-on-surface-variant group-hover/react:text-primary transition-colors text-center leading-tight">{{ __('Clean Code') }}</span>
                    </button>
                    <button wire:click="react('optimisable')" class="group/react flex flex-col items-center p-8 rounded-[2.5rem] border-2 transition-all duration-500 shadow-xl {{ $post->reactions->where('user_id', auth()->id())->where('type', 'optimisable')->count() ? 'bg-primary/20 border-primary shadow-[0_25px_50px_rgba(var(--primary-rgb),0.3)] scale-105' : 'bg-[#1e1f2b] border-white/5 hover:border-primary/50' }} hover:-translate-y-2 active:scale-95">
                        <span class="text-5xl mb-6 group-hover/react:scale-125 group-hover/react:rotate-6 transition-transform duration-500 drop-shadow-2xl">🚀</span>
                        <span class="text-[10px] font-black uppercase tracking-[0.25em] text-on-surface-variant group-hover/react:text-primary transition-colors text-center leading-tight">{{ __('Performance') }}</span>
                    </button>
                    <button wire:click="react('mindblown')" class="group/react flex flex-col items-center p-8 rounded-[2.5rem] border-2 transition-all duration-500 shadow-xl {{ $post->reactions->where('user_id', auth()->id())->where('type', 'mindblown')->count() ? 'bg-primary/20 border-primary shadow-[0_25px_50px_rgba(var(--primary-rgb),0.3)] scale-105' : 'bg-[#1e1f2b] border-white/5 hover:border-primary/50' }} hover:-translate-y-2 active:scale-95">
                        <span class="text-5xl mb-6 group-hover/react:scale-125 group-hover/react:-rotate-12 transition-transform duration-500 drop-shadow-2xl">🤯</span>
                        <span class="text-[10px] font-black uppercase tracking-[0.25em] text-on-surface-variant group-hover/react:text-primary transition-colors text-center leading-tight">{{ __('Architecture') }}</span>
                    </button>
                    <button wire:click="react('security')" class="group/react flex flex-col items-center p-8 rounded-[2.5rem] border-2 transition-all duration-500 shadow-xl {{ $post->reactions->where('user_id', auth()->id())->where('type', 'security')->count() ? 'bg-primary/20 border-primary shadow-[0_25px_50px_rgba(var(--primary-rgb),0.3)] scale-105' : 'bg-[#1e1f2b] border-white/5 hover:border-primary/50' }} hover:-translate-y-2 active:scale-95">
                        <span class="text-5xl mb-6 group-hover/react:scale-125 group-hover/react:rotate-12 transition-transform duration-500 drop-shadow-2xl">🛡️</span>
                        <span class="text-[10px] font-black uppercase tracking-[0.25em] text-on-surface-variant group-hover/react:text-primary transition-colors text-center leading-tight">{{ __('Security') }}</span>
                    </button>
                </div>
            </div>
            ...
            <div class="bg-[#1a1b26] rounded-[3.5rem] p-12 border border-white/5 shadow-[0_40px_100px_rgba(0,0,0,0.5)] group/author relative overflow-hidden transition-all hover:border-primary/30 active:scale-[0.98]">
                ...
                <div class="flex items-center gap-8 relative">
                    <div class="relative shrink-0">
                        <div class="absolute inset-0 bg-primary/20 blur-[30px] rounded-full scale-0 group-hover/author:scale-125 transition-transform duration-1000"></div>
                        ...
                        <div class="absolute -bottom-2 -right-2 bg-emerald-500 w-6 h-6 rounded-xl border-4 border-[#1a1b26] flex items-center justify-center shadow-xl z-20">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div class="flex">
                            <p class="text-[10px] font-black uppercase tracking-[0.3em] text-primary bg-primary/10 px-4 py-1.5 rounded-xl border border-primary/20 inline-block shadow-sm backdrop-blur-md">Pro Contributor</p>
                        </div>
                    </div>
                </div>
                <div class="mt-12 space-y-8 relative">
                    <div class="relative">
                        <div class="absolute -left-6 top-0 bottom-0 w-[2px] bg-gradient-to-b from-primary/40 to-transparent rounded-full opacity-50"></div>
                        <p class="text-base text-on-surface-variant leading-relaxed font-medium opacity-80 pl-2 italic">
                    ...
                    <div class="flex items-center justify-between pt-10 border-t border-white/5 relative">
                        <div class="text-center group-hover/author:-translate-y-2 transition-transform duration-500">
                            <p class="text-[10px] font-black uppercase tracking-[0.4em] text-on-surface-variant/40 mb-3">Impact</p>
                            <p class="font-display font-black text-4xl text-primary drop-shadow-[0_10px_20px_rgba(var(--primary-rgb),0.3)]">{{ $post->user->reputation_score }}</p>
                        </div>
                        <div class="h-12 w-px bg-white/5 opacity-40"></div>
                        <div class="text-center group-hover/author:-translate-y-2 transition-transform duration-500 delay-100">
                            <p class="text-[10px] font-black uppercase tracking-[0.4em] text-on-surface-variant/40 mb-3">Manifests</p>
                            <p class="font-display font-black text-4xl text-on-surface drop-shadow-2xl">{{ $post->user->posts->count() }}</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="p-16 space-y-16 relative">
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-16">
                        <div class="space-y-6">
                            <div class="flex items-center gap-4 pl-4">
                                ...
                                <span class="text-[11px] font-black uppercase tracking-[0.4em] text-on-surface-variant/40">{{ __('Current Implementation') }}</span>
                            </div>
                            <div class="bg-[#0f111a] border border-red-500/20 p-10 rounded-[3rem] font-mono text-[14px] text-red-500/50 line-through leading-[2.2rem] shadow-[inset_0_4px_20px_rgba(0,0,0,0.4)] transition-all group-hover/modal:border-red-500/30">
                                {{ $originalContent }}
                            </div>
                        </div>
                        <div class="space-y-6">
                             <div class="flex items-center gap-4 pl-4">
                                <div class="w-1 h-1 rounded-full bg-emerald-500 shadow-[0_0_10px_#4edea3]"></div>
                                <span class="text-[11px] font-black uppercase tracking-[0.4em] text-emerald-500">{{ __('Proposed refinement') }}</span>
                            </div>
                            <div class="relative group/edit">
                                <textarea wire:model="suggestedContent" 
                                          ...
                                          rows="4"></textarea>
                                <div class="absolute top-6 right-10 text-[10px] font-black uppercase tracking-[0.4em] text-primary/30 group-hover/edit:text-primary transition-colors italic">Interactive Layer</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-6 relative">
                         <div class="flex items-center gap-4 pl-4">
                            ...
                            <span class="text-[11px] font-black uppercase tracking-[0.4em] text-on-surface-variant/40">{{ __('Technical Justification') }}</span>
                        </div>
