...
## 2026-04-10-82 : Durcissement des Protocoles de S�curit� Karma (US33)
- **Auteur** : Antigravity
- **Statut** : ? Impl�ment�
- **Contexte** : N�cessit� de verrouiller herm�tiquement les interactions pour emp�cher tout contournement des barri�res de r�putation (Karma), notamment via les raccourcis clavier (touche Entr�e) et les appels API directs.
- **D�cision** :
    1. **S�curisation Backend Syst�matique** : Int�gration de gardes de permission Karma (hasKarmaPermission) dans toutes les m�thodes interactives de PostDetail (Commentaires, Suggestions, Reviews, Votes, Likes) et Feed (Votes).
    2. **Neutralisation des Bypass Frontend** : Refonte des gestionnaires d'�v�nements wire:keydown.enter dans les templates Blade avec des conditions Alpine.js pour bloquer physiquement le d�clenchement des requ�tes en cas de Karma insuffisant.
    3. **Unification du Feedback (VibeAction)** : Utilisation syst�matique du trait HasVibeNotifications pour transformer les exceptions d'autorisation en notifications Toast 'Premium' (avec retour sonore et visuel), garantissant une exp�rience transparente et �ducative.
    4. **Audit d'Int�grit� des R�les** : V�rification compl�te de la cha�ne de permissions (Unranked -> Contributor -> Reviewer -> Expert -> Elite) pour assurer la coh�rence des droits de mod�ration et de contribution.
- **Impact** : Protection totale de l'int�grit� des donn�es, �limination des vecteurs d'attaque par simulateur de clavier et renforcement de l'aspect m�ritocratique de la plateforme.

# [085] 2026-04-10 : Synchronisation de l'Activité et de la Réputation en Temps Réel
- **Auteur** : Antigravity
- **Statut** : ✅ Implémenté
- **Contexte** : Les statistiques du profil (heatmap d'activité et score de réputation) ne se mettaient pas à jour dynamiquement lors des interactions sur le site.
- **Décisions** :
    1. **Automatisation des Contributions** : Ajout d'une méthode `UserContribution::record()` et injection dans tous les points d'interaction (Post, Commentaire, Suggestion, Revue, Réaction).
    2. **Recalibrage de la Réputation** : Correction de la méthode `recalculateReputationScore` dans `User.php` pour s'aligner sur les types de réactions réels (`mindblown`, `optimisable`).
