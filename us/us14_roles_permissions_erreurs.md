...
*   **Guest (Visiteur non authentifié)**
    *   **Actions Autorisées** : Voir les "Vibes" publiques (Posts), s'authentifier via GitHub, lire les pages d'information (Landing Page).
    *   **Actions Interdites** : Créer des posts, voter, commenter, rejoindre des laboratoires (Groupes), accéder aux posts privés, accéder à l'API de modification.

*   **User (Membre authentifié)**
    *   **Actions Autorisées** : Créer des posts publics/privés, voter et donner des retours (reviews), rejoindre des laboratoires collaboratifs, voir les posts privés des laboratoires dont il est membre, éditer/supprimer ses propres posts.
    *   **Actions Interdites** : Supprimer le post d'un autre utilisateur, forcer l'entrée dans un laboratoire privé, supprimer un laboratoire qu'il n'a pas créé.

*   **Lab Owner (Propriétaire de groupe/laboratoire)**
    *   **Actions Autorisées** : Hérite des droits du *User*. Peut modifier les paramètres du laboratoire, expulser des membres, supprimer le laboratoire.
    *   **Actions Interdites** : Interférer avec la modération globale de la plateforme, éditer les posts privés publiés dans son laboratoire par d'autres utilisateurs.

*   **Admin (Administrateur système)**
    *   **Actions Autorisées** : Modérer n'importe quel post, supprimer n'importe quel commentaire/review, dissoudre des laboratoires qui ne respectent pas les règles, gérer les rôles des utilisateurs.
...
*   *Scénario* : Un **User A** tente d'accéder (via l'URL ou l'API) à un "Vibe" privé appartenant à un **Lab X**, alors qu'il n'est pas membre de ce lab.
*   *Comportement* : 
...
*   **Contexte** : L'action est valide syntaxiquement mais viole une règle métier du produit (ex: Tenter de liker son propre post de manière forcée si désactivé au niveau core, rejoindre un lab déjà complet).
*   **Couverture** : Exceptions spécifiques lancées depuis les classes *Actions* (ex: `ActionDeniedException`).
*   **Comportement Interface** : Toast d'erreur informatif expliquant *pourquoi* la règle empèche l'action : "Ce lab a atteint sa capacité maximale d'analystes." Redirection douce.
